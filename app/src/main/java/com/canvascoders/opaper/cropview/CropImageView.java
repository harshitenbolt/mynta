package com.canvascoders.opaper.cropview;
...
import com.canvascoders.opaper.cropview.animation.SimpleValueAnimator;
import com.canvascoders.opaper.cropview.animation.SimpleValueAnimatorListener;
import com.canvascoders.opaper.cropview.animation.ValueAnimatorV14;
import com.canvascoders.opaper.cropview.animation.ValueAnimatorV8;
import com.canvascoders.opaper.cropview.callback.Callback;
...
@SuppressWarnings("unused") public class CropImageView extends androidx.appcompat.widget.AppCompatImageView {
  ...
  private void handleStyleable(Context context, AttributeSet attrs, int defStyle, float mDensity) {
    ...
    try {
      drawable = ta.getDrawable(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_img_src);
      if (drawable != null) setImageDrawable(drawable);
      for (CropMode mode : CropMode.values()) {
        if (ta.getInt(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_crop_mode, 3) == mode.getId()) {
          mCropMode = mode;
          break;
        }
      }
      mBackgroundColor =
          ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_background_color, TRANSPARENT);
      mOverlayColor =
          ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_overlay_color, TRANSLUCENT_BLACK);
      mFrameColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_frame_color, WHITE);
      mHandleColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_color, WHITE);
      mGuideColor = ta.getColor(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_color, TRANSLUCENT_WHITE);
      for (ShowMode mode : ShowMode.values()) {
        if (ta.getInt(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_show_mode, 1) == mode.getId()) {
          mGuideShowMode = mode;
          break;
        }
      }
      ...
      for (ShowMode mode : ShowMode.values()) {
        if (ta.getInt(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_show_mode, 1) == mode.getId()) {
      ...
      mHandleSize = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_size,
          (int) (HANDLE_SIZE_IN_DP * mDensity));
      mTouchPadding = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_touch_padding, 0);
      mMinFrameSize = ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_min_frame_size,
          (int) (MIN_FRAME_SIZE_IN_DP * mDensity));
      mFrameStrokeWeight =
          ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_frame_stroke_weight,
              (int) (FRAME_STROKE_WEIGHT_IN_DP * mDensity));
      mGuideStrokeWeight =
          ta.getDimensionPixelSize(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_guide_stroke_weight,
              (int) (GUIDE_STROKE_WEIGHT_IN_DP * mDensity));
      mIsCropEnabled = ta.getBoolean(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_crop_enabled, true);
      mInitialFrameScale = constrain(
          ta.getFloat(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_initial_frame_scale,
              DEFAULT_INITIAL_FRAME_SCALE), 0.01f, 1.0f, DEFAULT_INITIAL_FRAME_SCALE);
      mIsAnimationEnabled =
          ta.getBoolean(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_animation_enabled, true);
      mAnimationDurationMillis = ta.getInt(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_animation_duration,
          DEFAULT_ANIMATION_DURATION_MILLIS);
      mIsHandleShadowEnabled =
          ta.getBoolean(com.isseiaoki.simplecropview.R.styleable.scv_CropImageView_scv_handle_shadow_enabled, true);
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
      ta.recycle();
